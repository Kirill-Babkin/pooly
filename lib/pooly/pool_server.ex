defmodule Pooly.PoolServer do
  ...
  def checkin(pool_name) do
    GenServer.cast(name(pool_name), :checkin)
  end
  ...
  def handle_info({:DOWM, ref, _,_,_}, state = %{monitors: monitors, workers: workers}) do
  ...
  def handle_info({:EXIT, pid, reason}, state = %{worker_sup: worker_sup}) when pid == worker_sup do
    {:stop, reason, state}
  end
  ...
  def prepopulate(size, _sup, workers) when size < 1 do
    workers
  end

  def prepopulate(size, sup, workers) do
